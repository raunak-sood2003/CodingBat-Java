public class Recursion {
	...
	public String pairStar(String str) {
		  ...
		  {
		    str = str.charAt(0)+"*"+str.charAt(1)+pairStar(str.substring(2));
		    pairStar(str);
		  }
	...
	public int countHi2(String str) {
		  if (str.length()<2 || str.equals("xxxx"))
		  {
		    return 0;
		  }
	 ...
	 * the count of the occurrences of 8 as a digit, except that an 
	 * 8 with another 8 immediately to its left counts double, so 
	 * 8818 yields 4. Note that mod (%) by 10 yields the rightmost 
	 ...
	 * times 10. We'll use the convention of considering only the 
	...
	/*
	 * The fibonacci sequence is a famous bit of mathematics, and 
	 * it happens to have a recursive definition. The first two values 
	 ...
	 * fibonacci number, with n=0 representing the start of the 
	 * sequence.
	 */
	 ...
	 * where all the lowercase 'x' chars have been changed to 'y' 
	 * chars.
	 */
	 ...
	 * Pair's can overlap, so "AxAxA" contains 3 pairs -- 2 for A 
	 * and 1 for x. Recursively compute the number of pairs in the 
	 * given string.
	 */
	...
	/*
	 * Given a string and a non-empty substring sub, compute 
	 * recursively the largest substring which starts and ends with 
	...
	public int strDist(String str, String sub) {
		  ...
		  if (str.substring(0, len_sub).equals(sub))
		  {
		    if (str.substring(len_str-len_sub,len_str).equals(sub))
		    {
		      return len_str;
		    }
		    return strDist(str.substring(0,len_str-1),sub);
		  }
